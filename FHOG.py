...
@jit
def func2(dx, dy, boundary_x, boundary_y, r, alfa, nearest, w, k,
          height, width, sizeX, sizeY, p, stringSize):
    mapp = np.zeros((sizeX * sizeY * p), np.float32)
    for i in range(sizeY):
        for j in range(sizeX):
            for ii in range(k):
                for jj in range(k):
                    if ((i * k + ii > 0) and (i * k + ii < height - 1) and
                        (j * k + jj > 0) and (j * k + jj < width - 1)):
                        ...
                        if ((j + nearest[jj] >= 0) and (j + nearest[jj] <= sizeX - 1)):
                            ...
                            mapp[i * stringSize + (j + nearest[jj]) * p + alfa[k * i + ii, j * k + jj, 1] + NUM_SECTOR] += \
                                r[k * i + ii, j * k + jj] * w[ii, 0] * w[jj, 1]
                        if ((i + nearest[ii] >= 0) and (i + nearest[ii] <= sizeY - 1) and
                            (j + nearest[jj] >= 0) and (j + nearest[jj] <= sizeX - 1)):
    ...
    for i in range(sizeY):
        for j in range(sizeX):
            ...
            for jj in range(2 * xp):
                newData[pos2 + jj] = np.sum(
                    mappmap[pos1 + yp * xp + jj: pos1 + 3 * yp * xp + jj: 2 * xp]
                ) * ny
            for jj in range(xp):
                newData[pos2 + 2 * xp + jj] = np.sum(
                    mappmap[pos1 + jj: pos1 + jj + yp * xp: xp]
                ) * ny
            for ii in range(yp):
                newData[pos2 + 3 * xp + ii] = np.sum(
                    mappmap[pos1 + yp * xp + ii * xp * 2: pos1 + yp * xp + ii * xp * 2 + 2 * xp]
                ) * nx
    return newData
...
def getFeatureMaps(image, k, mapp):
    ...
    a_x = np.concatenate((half_k - np.arange(half_k) - 0.5,
                          np.arange(half_k, k) - half_k + 0.5)).astype(np.float32)
    b_x = np.concatenate((half_k + np.arange(half_k) + 0.5,
                          -np.arange(half_k, k) + half_k - 0.5 + k)).astype(np.float32)
    w[:, 0] = 1.0 / a_x * ((a_x * b_x) / (a_x + b_x))
    w[:, 1] = 1.0 / b_x * ((a_x * b_x) / (a_x + b_x))
    mapp['map'] = func2(dx, dy, boundary_x, boundary_y, r, alfa, nearest, w, k,
                        height, width, sizeX, sizeY, p, stringSize)
    return mapp
